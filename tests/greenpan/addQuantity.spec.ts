...
test.describe("Greenpan Quantity Validation", () => {
  let po: PoManager;
  ...
  test(
    "Verify system does not accept quantity less than minimum required (2 KG)",
  ...
  test(
    "Verify system accepts quantity equal or greater than minimum required (2 KG)",
  ...
  test("Verify system deducts quantity from remaining points", { tag: ["@greenpan", "@e2e","@regression"] }, async () => {
  ...
  test(
    "Verify Add Gift button becomes disabled when remaining points reach zero",
    { tag: ["@greenpan", "@e2e","@regression"] },
    async () => {
